...
for i in np.arange(len(mh_low_arr)):
    ...
    for j in np.arange(len(simulations)):
        for k in np.arange(len(snap)):
            ...
            print("sim",simulations[j],nprofs)
'''
            ...
            for n in np.arange(nprofs):
                ax1.loglog(r_mpc_cut2,val_dens[n,:],alpha=0.2)
                ax2.loglog(r_mpc_cut2,val_pres[n,:],alpha=0.2)
                ax3.loglog(r_mpc_cut2,val_metal_gmw[n,:],alpha=0.2)
                ax4.loglog(r_mpc_cut2,val_temp_gmw[n,:],alpha=0.2)
            ax1.loglog(r_mpc_cut2,mean_unnorm_dens,color='k')
            ax1.loglog(r_mpc_cut2,val_dens[3,:],color='r')
            ax2.loglog(r_mpc_cut2,mean_unnorm_pres,color='k',label='mean with all halos')
            ax2.loglog(r_mpc_cut2,val_pres[3,:],color='r',label='problem halo')
            ax3.loglog(r_mpc_cut2,mean_unnorm_metal_gmw,color='k')
            ax3.loglog(r_mpc_cut2,val_metal_gmw[3,:],color='r')
            ax4.loglog(r_mpc_cut2,mean_unnorm_temp_gmw,color='k')
            ax4.loglog(r_mpc_cut2,val_temp_gmw[3,:],color='r')
            
            val_dens,val_pres,val_metal_gmw,val_temp_gmw=list(val_dens),list(val_pres),list(val_metal_gmw),list(val_temp_gmw)
            del val_dens[3]
            del val_pres[3]
            del val_metal_gmw[3]
            del val_temp_gmw[3]
            
            mean_dens=np.apply_along_axis(lambda v: np.mean(v[np.nonzero(v)]),0,val_dens)
            mean_pres=np.apply_along_axis(lambda v: np.mean(v[np.nonzero(v)]),0,val_pres)
            mean_metal_gmw=np.apply_along_axis(lambda v: np.mean(v[np.nonzero(v)]),0,val_metal_gmw)
            mean_temp_gmw=np.apply_along_axis(lambda v: np.mean(v[np.nonzero(v)]),0,val_temp_gmw)
            ax1.loglog(r_mpc_cut2,mean_dens,color='b')
            ax2.loglog(r_mpc_cut2,mean_pres,color='b',label='mean w/o prob halo')
            ax3.loglog(r_mpc_cut2,mean_metal_gmw,color='b')
            ax4.loglog(r_mpc_cut2,mean_temp_gmw,color='b')
            
            
            ax1.set_title(r"Mass $%s \leq M_\odot \leq %s, z=%.2f$"%(mh_low_pow[i],mh_high_pow[i],z),size=14)
            ax2.set_title(r" %i profiles"%nprofs)
...
#plt.savefig(home+'Figures/mean_halo_check_test.png',bbox_inches='tight')
'''
